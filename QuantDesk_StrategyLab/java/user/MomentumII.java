...
public class MomentumII implements TradingStrategy {
  ...
  private static final Indicator UpperChannel = new MovingMaximum(5,false);
  private static final Indicator LowerChannel = new MovingMinimum(5,false);
  ...
  public MomentumII(double sizingFactor) {
  ...
  protected void placeBuyOrders(final StockMarketHistory histories,
    ...
    candidates = candidates.subList(0, candidates.size()/4);

    // sort by long momentum
    Collections.sort(candidates, new Comparator<StockHistory>() {
      public int compare(StockHistory history1, StockHistory history2) {
          double mo1 = momentum(date, history1, 100);
          double mo2 = momentum(date, history2, 100);
    ...
    for (StockHistory history : candidates) {
      ...
      double projectedPrice = sdp.getAdjustedHigh();
