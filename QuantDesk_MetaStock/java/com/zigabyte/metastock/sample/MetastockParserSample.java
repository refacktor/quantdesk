...
/** Runs {@link MetastockParser} to parse Metastock files
    under a root directory, then displays a window listing the stocks
    and graphing one chosen history. **/
public class MetastockParserSample extends JFrame {
  /** Starts metastock parser on root directory parameter and loads all
      stock data into memory, then displays a window where the user
      can select a stock and see a candlestick graph of its loaded history.
      
      @param parameters path to root directory of metastock files.
      (Concatenates parameters with spaces, so ok to type path with
  ...
  public static void main(String[] parameters) throws IOException {
    ...
    if (parameters.length == 0) {
      System.err.println("parameter: /path/to/Metastock/directory");
      System.exit(-1);
      return;
    } else {
      // concatenate names divided by spaces
      StringBuffer buf = new StringBuffer();
      for (int i = 0; i < parameters.length; i++) {
	if (i > 0) buf.append(' ');
	buf.append(parameters[i]);
      }
      metastockDir = new File(buf.toString());
    }	
    ...
    long memoryBefore = occupiedMemory();

    StockMarketHistory stockMarketHistory =
      factory.loadHistory(metastockDir);

    long dataMemory = occupiedMemory() - memoryBefore;
    System.out.println("Data occupies about "+
		       NumberFormat.getInstance().format(dataMemory)+
		       " bytes in memory");
    
    // Start the window on the data
    JFrame sample = new MetastockParserSample(stockMarketHistory);
    sample.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
    sample.setLocationRelativeTo(null); // center on screen
  ...
  public MetastockParserSample(StockMarketHistory stockMarketHistory) {
    super("MetastockParserSample");
    initLayout();
