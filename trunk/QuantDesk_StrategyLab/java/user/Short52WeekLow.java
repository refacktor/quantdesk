...
//	private static final Indicator ISupportLevel = new MovingMinimum(20, false, FieldID.LOW);
	private static final Indicator I52WeekLow = new MovingMinimum(365, true, FieldID.LOW);
	final int nStocks = 10;

	public Short52WeekLow() {
	}
	...
	private void doBuys(final StockMarketHistory histories, final TradingAccount account, final Date date, double buyingPower) {
		...
		while(buyingPower >= positionSize && candidates.size() > 0) {
			...
			if(account.getStockPosition(candidate.getSymbol()) == null) {
				double projectedPrice = candidate.get(date).getAdjustedLow();
				int nShares = (int) (positionSize / projectedPrice);

				if (nShares > 0) {
					account.sellStock(candidate.getSymbol(), nShares,
	...
	private List<StockHistory> getNewLows(final Date date, List<StockHistory> liquidList) {
		...
		for (StockHistory history : liquidList) {
			...
			if (index >= 0) { // traded today?
				...
				double yday52wkLow = I52WeekLow.compute(history, index-1);

				// close higher than yesterday's 52-week high?
				if(history.get(index).getAdjustedClose() < yday52wkLow) {		
