...
public class ComputeCorrelations
{
	...
	private void connectDB()
	{
		try
		{
			...
			con = DriverManager.getConnection("jdbc:mysql://"+host+"/"+name+"?user="+login+"&useServerPrepStmts=false&autoReconnect=true&password="+password);
	...
	private int computeCorrelation( int stock1 )
	{
		...
		try
		{
			...
			while( rs.next() )
			{
				...
				quotes.add( ddp );
                counter++;
                if(counter%100000 == 0) System.err.println( counter + " passed");
			}
			rs.close();
			if( id != 0 )
			{
